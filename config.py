...
class Config:
    ...
    REMINDER_MINUTES_BEFORE = int(os.environ.get('REMINDER_MINUTES_BEFORE', 60))
    SCHEDULER_INTERVAL_MINS = 15


class DevConfig(Config):
    DEBUG = True
    # ── Falls back to SQLite — no Postgres install needed for local dev ──────
    # Set DATABASE_URL in your .env when you want to use Postgres
    SQLALCHEMY_DATABASE_URI = os.environ.get(
        'DATABASE_URL',
        f'sqlite:///{os.path.join(_root, "dozo_dev.db")}'
    )
    # SQLite doesn't support pool options — clear them for dev
